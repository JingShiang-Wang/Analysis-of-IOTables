...
在這份報告中，我們針對產業關聯程度表用了兩種分析方法，這些資料每5年一份總共8份，第一種方法是用Cauchy–Schwarz inequality 來找最佳解；第二種方法我們先定義了感應度以及影響度，再把它們依照年份、產業標在圖上並進行分類，最後還有將這些資料用MDS的方法進行資料的可視化，最後根據這些分類作出一些結論。
